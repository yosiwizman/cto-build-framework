...
| responsive-002 | Responsive UI | Human re-execution | Class 1 (intended) — **source artifact not yet confirmed** | Original Task Tracker + Contact Manager HTML files (not yet filed in repo) | `validation/runs/responsive-ui-human-reexecution-planned.md` | None — run not yet executed | None — pending execution | None — pending execution | **Planned** | Protocol filed; quick guide and live capture sheet created; source artifact class will be confirmed when original HTML files are located; if run against reconstructed-apps/ files, must be re-classified as Class 2 source / Class 3 result and use `reconstructed-run-wrapper-template.md` |
